...
public abstract class SfTask extends Task {

    public class JsonParser implements ISfJsonParser {
        ...
        public void log(final String message, final int level) {
            if (message != null && !message.isEmpty()) {
                int l = level;
                if (SfTask.this.getQuiet() && level < Project.MSG_VERBOSE) {
                    l = Project.MSG_VERBOSE;
                }
                SfTask.this.log(message, l);
            }
        }
        ...
        public void parse(final JSONObject json) {
            if (json != null) {
                if (SfTask.this.getJsonProperty() != null && !SfTask.this.getJsonProperty().isEmpty()) {
                    SfTask.this.getProject().setNewProperty(SfTask.this.getJsonProperty(), json.toString());
                }
        ...
        public void setErrorMessage(final String message) {
            if (SfTask.this.getFailOnError() && !SfTask.this.hasErrorMessage()) {
                SfTask.this.setErrorMessage(message.trim());
            }
        }
        ...
        protected void doParse(final JSONObject json) {
            ...
            if (SfTask.this.getStatusProperty() != null && !SfTask.this.getStatusProperty().isEmpty()) {
                SfTask.this.getProject().setNewProperty(SfTask.this.getStatusProperty(), Integer.toString(status));
            }
            ...
            if (result == null) {
                ...
                if (results != null) {
                    String property = SfTask.this.getResultProperty();
                    if (property == null) {
                        property = "";
                    }
            ...
            if (message != null && !message.isEmpty()) {
                ...
                if (status > 0 && SfTask.this.getFailOnError()) {
                    SfTask.this.setErrorMessage(message);
                }
            }
        ...
        protected void handleValue(final String property, final String key, final String value) {
            if (SfTask.this.getResultProperty() != null && !SfTask.this.getResultProperty().isEmpty()) {
                SfTask.this.getProject().setNewProperty(property + "." + key, value);
            }
        }
    ...
    public void execute() throws BuildException {
        ...
        for (int i = command.length - 1; i >= 0; i--) {
            this.getCommandline().createArgument(true).setValue(command[i]);
        }
        this.createArguments();

        try {
            final Project p = getProject();
            final Execute exe = new Execute(new SfOutputParser(getParser()));
            exe.setAntRun(p);
            exe.setEnvironment(new String[]{"SF_AUTOUPDATE_DISABLE=true"});
    ...
    protected void checkConfiguration() {
        if (this.getExecutable() == null) {
            this.setExecutable("sf");
            if (IS_DOS) {
                final String path = System.getenv("PATH");
                final String pathSeparator = System.getProperty("path.separator");

                for (final String pathElement : path.split(pathSeparator)) {
                    final File command = new File(pathElement, "sf.cmd");
                    if (command.isFile()) {
                        this.setExecutable("sf.cmd");
                        break;
                    }
                    final File executable = new File(pathElement, "sf.exe");
                    if (executable.isFile()) {
                        this.setExecutable("sf.exe");
                        break;
                    }
                }
            }
