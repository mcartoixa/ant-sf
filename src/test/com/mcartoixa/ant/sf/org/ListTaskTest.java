...
public class ListTaskTest {
    ...
    @Before
    public void setUp() {
        buildRule.configureProject("src/test/com/mcartoixa/ant/sf/org/list.xml", Project.MSG_DEBUG);
    }
    ...
    public void executeShouldLogOrgSummary() {
        buildRule.executeTarget("execute");
        Assert.assertTrue("Non scratch orgs should be logged", buildRule.getLog().contains("ant-sf (00000000000000000H)  [test@ant-sf.org]"));
        Assert.assertTrue("Scratch orgs should be logged", buildRule.getLog().contains("\t- ant-sf-scratch (000000000000000000) Ant SF [test@ant-sf.org]"));
    }
