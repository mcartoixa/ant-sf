...
if [ ! -f $PMD_HOME/bin/pmd ]; then
    wget -nv $_wget_interactive_options -O .tmp/pmd-dist-$_PMD_VERSION-bin.zip https://github.com/pmd/pmd/releases/download/pmd_releases%2F$_PMD_VERSION/pmd-dist-$_PMD_VERSION-bin.zip
    unzip .tmp/pmd-dist-$_PMD_VERSION-bin.zip -d .tmp
fi
...
if [ ! -d $JAVA_HOME ]; then error "Could not find Java JDK 11"; fi
